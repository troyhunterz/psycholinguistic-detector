...
# merging
news = pd.concat(
    [buzzfeed_fake, buzzfeed_real, politifact_fake, politifact_real], ignore_index=True
)
...
if __name__ == "__main__":
    ...
    max_rational = len(non_rational[non_rational["label"] == "fear_appeal"]) * 4

    rational = df[df["label"] == "rational_argument"].sample(n=max_rational, random_state=42)

    df_balanced = pd.concat([non_rational, rational], ignore_index=True)
    df_balanced = df_balanced.sample(frac=1, random_state=42)

    df_balanced[["text", "label"]].to_csv("data/processed/combined_en.csv", index=False)
