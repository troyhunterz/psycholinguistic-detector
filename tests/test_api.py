...
def test_predict_fear_appeal():
    response = client.post(
        "/predict",
        json={"text": "Enemies want to destroy everything we love! Our children are in danger!"},
    )
...
def test_predict_rational():
    response = client.post(
        "/predict",
        json={"text": "According to Federal Reserve data, inflation decreased by 2.3 percent."},
    )
    assert response.status_code == 200
    data = response.json()
    assert "label" in data
